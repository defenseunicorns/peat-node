// Package peat provides an idiomatic Go client for the peat-sidecar gRPC API.
//
// The client communicates with a co-located peat-sidecar process over Unix
// socket or TCP. The sidecar is a full CRDT mesh participant; this client
// provides a thin, pure-Go wrapper with channels for subscriptions and
...
func Connect(target string) (*Client, error) {
	...
	if strings.HasPrefix(target, "unix://") {
		...
		// h2c transport for Unix sockets (HTTP/2 cleartext, required by tonic)
		...
		// Connect RPC needs an HTTP base URL even for Unix sockets
		target = "http://localhost"
	} else {
		// h2c transport for TCP (tonic serves HTTP/2 without TLS)
		httpClient = &http.Client{
			Transport: &http2.Transport{
				AllowHTTP: true,
	...
	// Use gRPC protocol to match tonic's server
